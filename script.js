...
function initPhysics() {
    ...
    const asteroidAsteroidContact = new CANNON.ContactMaterial(
        asteroidMaterial,
        asteroidMaterial,
        {
            friction: 0.0,
...
function loadModels(manager) {
    ...
    gltfLoader.load('https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Duck/glTF/Duck.gltf', function(gltf) {
        models.playerShip = gltf.scene;
        models.playerShip.scale.set(0.5, 0.5, 0.5);
        models.playerShip.traverse(function(child) {
            if (child.isMesh) {
    ...
    for (let i = 1; i <= 3; i++) {
        gltfLoader.load(`https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Rock/glTF/Rock.gltf`, function(gltf) {
            models[`asteroid${i}`] = gltf.scene;
            models[`asteroid${i}`].scale.set(0.5, 0.5, 0.5);
            models[`asteroid${i}`].traverse(function(child) {
                if (child.isMesh) {
                    child.castShadow = true;
                    child.receiveShadow = true;
    ...
    // Load enemy ship models
    gltfLoader.load('https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Duck/glTF/Duck.gltf', function(gltf) {
        models.enemyShip = gltf.scene;
        models.enemyShip.scale.set(0.4, 0.4, 0.4);
        models.enemyShip.traverse(function(child) {
            if (child.isMesh) {
                child.castShadow = true;
                child.receiveShadow = true;
                child.material.envMapIntensity = 1.5;
                // Add red glow to enemy ships
                if (child.material) {
    ...
    weaponModels.forEach(weaponName => {
        gltfLoader.load('https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/BoxInterleaved/glTF/BoxInterleaved.gltf', function(gltf) {
            models[weaponName] = gltf.scene;
            models[weaponName].scale.set(0.2, 0.2, 0.5);
            models[weaponName].traverse(function(child) {
                if (child.isMesh) {
                    child.castShadow = true;
                    child.material.envMapIntensity = 1.5;
                }
    ...
    // Load powerup model
    gltfLoader.load('https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Cube/glTF/Cube.gltf', function(gltf) {
        models.powerup = gltf.scene;
        models.powerup.scale.set(0.3, 0.3, 0.3);
        models.powerup.traverse(function(child) {
            if (child.isMesh) {
                child.castShadow = true;
...
function loadTextures(manager) {
    ...
    textures.spaceBackground = textureLoader.load('https://images.pexels.com/photos/998641/pexels-photo-998641.jpeg');
    textures.spaceBackground.encoding = THREE.sRGBEncoding;
    
    // Load star field
    textures.starField = textureLoader.load('https://images.pexels.com/photos/1169754/pexels-photo-1169754.jpeg');
    textures.starField.wrapS = THREE.RepeatWrapping;
    textures.starField.wrapT = THREE.RepeatWrapping;
    
    // Load planet textures
    textures.planet1 = textureLoader.load('https://images.pexels.com/photos/87651/earth-blue-planet-globe-planet-87651.jpeg');
    textures.planet1.encoding = THREE.sRGBEncoding;
    
    // Load explosion texture
    textures.explosion = textureLoader.load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/sprites/spark1.png');
    
    // Load particle textures
    textures.particle = textureLoader.load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/sprites/circle.png');
    
    // Load laser textures
    textures.laser = textureLoader.load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/sprites/disc.png');
    ...
    const envMapLoader = new THREE.RGBELoader(manager);
    envMapLoader.load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/equirectangular/venice_sunset_1k.hdr', function(texture) {
...
function loadAudio(manager) {
    // Create Howler sound sprites
    sounds.laser = new Howl({
        src: ['https://freesound.org/data/previews/495/495005_9097153-lq.mp3'],
        volume: 0.5,
        rate: 1.5
    });
    
    sounds.plasma = new Howl({
        src: ['https://freesound.org/data/previews/387/387232_7255534-lq.mp3'],
        volume: 0.6
    });
    
    sounds.missile = new Howl({
        src: ['https://freesound.org/data/previews/404/404754_140737-lq.mp3'],
        volume: 0.7
    });
    
    sounds.quantum = new Howl({
        src: ['https://freesound.org/data/previews/396/396331_7255534-lq.mp3'],
        volume: 0.8
    });
    
    sounds.explosion = new Howl({
        src: ['https://freesound.org/data/previews/235/235968_4291669-lq.mp3'],
        volume: 0.6
    });
    
    sounds.hit = new Howl({
        src: ['https://freesound.org/data/previews/331/331912_5121236-lq.mp3'],
        volume: 0.5
    });
    
    sounds.playerHit = new Howl({
        src: ['https://freesound.org/data/previews/331/331912_5121236-lq.mp3'],
        volume: 0.7,
        rate: 0.8
    });
    ...
    sounds.reload = new Howl({
        src: ['https://freesound.org/data/previews/522/522407_10058132-lq.mp3'],
        volume: 0.6
    });
    
    sounds.powerup = new Howl({
        src: ['https://freesound.org/data/previews/320/320181_5260872-lq.mp3'],
        volume: 0.7
    });
    
    sounds.levelUp = new Howl({
        src: ['https://freesound.org/data/previews/320/320655_5260872-lq.mp3'],
        volume: 0.8
    });
    
    // Background music using Tone.js
    const backgroundMusic = new Tone.Player({
        url: 'https://freesound.org/data/previews/451/451547_6142149-lq.mp3',
        loop: true,
        volume: -10,
        autostart: false
    }).toDestination();
...
function createLights() {
    ...
    const textureFlare1 = textureLoader.load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/lensflare/lensflare1.png');
...
function initControls() {
    ...
    document.addEventListener('mousedown', () => { gameState.mouseDown = true; });
    document.addEventListener('mouseup', () => { gameState.mouseDown = false; });
}
...
function createAsteroid() {
    ...
    asteroidBody.velocity.set(
        (dirX / length) * speed * 60,
        (dirY / length) * speed * 60,
        (dirZ / length) * speed * 60
    );
    ...
    // Add asteroi.addBody(asteroidBody);
    asteroidBodies.push(asteroidBody);
...
function hasActivePowerup(name) {
    ...
    return gameState.activePowerups.some(powerup => 
        powerup.name === name && powerup.endTime > now
    );
}
